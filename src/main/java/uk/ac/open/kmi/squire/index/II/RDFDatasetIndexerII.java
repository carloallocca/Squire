...
public class RDFDatasetIndexerII {
	...
	public void addSignature(String urlAddress, String graphName, ArrayList<String> classSet,
			ArrayList<String> objectPropertySet, ArrayList<String> datatypePropertySet, ArrayList<String> individualSet,
			ArrayList<String> literalSet, ArrayList<String> rdfVocabulary, ArrayList<String> propertySet)
			throws IOException, CorruptIndexException, LockObtainFailedException, Exception {
		if (!(_alreadyExists(urlAddress, graphName))) {
			...
			StringField dPropertySet = new StringField("DatatypePropertySet", datatypePropertySet.toString(),
					Field.Store.YES);
	...
	private IndexWriter getIndexWriter() throws LockObtainFailedException, IOException {
		if (indexWriter == null) {
			...
			String fileSystemNotFoundExceptionReason = "[RDFDatasetIndexerII::getIndexWriter] The file system, identified by the "
					+ this.indexDirectoryPath + "does not exist and cannot be created automatically.";
			String illegalArgumentExceptionReason = "[RDFDatasetIndexerII::getIndexWriter] Wrong format of the"
					+ this.indexDirectoryPath + "parameter.";
			Analyzer analyzer = new StandardAnalyzer();
			try {
				path = Paths.get(this.indexDirectoryPath);
				indexDirectory = FSDirectory.open(path, NativeFSLockFactory.INSTANCE);// The primary benefit of
																						// NativeFSLockFactory is that
																						// locks (not the lock file
																						// itsself) will be properly
																						// removed (by the OS) if the
																						// JVM has an abnormal exit.
				try {
					...
					config.setOpenMode(
							IndexWriterConfig.OpenMode.CREATE_OR_APPEND);/*
																			 ...
																			 * CREATE_OR_APPEND if used IndexWriter will
																			 * create a new index if there is not
																			 * already an index at the provided path and
																			 * otherwise open the existing index.
																			 */
				...
				Logger.getLogger(RDFDatasetIndexerII.class.getName()).log(Level.SEVERE, illegalArgumentExceptionReason,
						iae);
