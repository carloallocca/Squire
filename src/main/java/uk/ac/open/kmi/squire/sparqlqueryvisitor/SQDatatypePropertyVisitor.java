...
public class SQDatatypePropertyVisitor extends ElementVisitorBase {
    ...
    public SQDatatypePropertyVisitor(IRDFDataset d1) {
        if (d1 == null) {
            throw new IllegalStateException("[SQDatatypePropertyVisitor]The IRDFDataset d1 is null!!");
        }
        this.d = d1;
        this.datasetDatatypePropertySet = d1.getDatatypePropertySet();
    }
...
//        System.out.println("[SQInstantiationVisitor::visit(ElementPathBlock el)] ");
        if (el == null) {
            throw new IllegalStateException("[SQObjectPropertyVisitor::visit(ElementPathBlock el)] The ElementPathBlock is null!!");
        }
        ...
        while (it.hasNext()) {
            ...
            // PREDICATE            
            if (predicate.isURI()) {
                if (this.datasetDatatypePropertySet.contains(predicate.getURI())) {
                    this.queryDatatypePropertySet.add(predicate.getURI());
                }
            }
        }
    }

    public Set<String> getQueryDatatypePropertySet() {
        return queryDatatypePropertySet;
    }
