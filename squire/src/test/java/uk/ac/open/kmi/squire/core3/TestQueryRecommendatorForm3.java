...
public class TestQueryRecommendatorForm3 {
	...
	public void testRecommend() {
		...
		try {
			...
			// + "}";
			String qString = "prefix rdfs:<http://www.w3.org/2000/01/rdf-schema#> "
					+ "prefix rdf:<http://www.w3.org/1999/02/22-rdf-syntax-ns#> " + " SELECT DISTINCT ?mod ?title "
					+ "where { " + " ?mod a <http://purl.org/vocab/aiiso/schema#Module>. "
					+ " ?mod <http://purl.org/dc/terms/title> ?title . " + "}";
			...
			QueryRecommendatorForm3 instance = new QueryRecommendatorForm3(qString, d1, d2, resultTypeSimilarityDegree,
			...
			System.out
					.println("////////////// PRINTING THE " + result.size() + " RECOMMENDATED QUERIES ///////////////");
			System.out.println("");
			System.out.println("");
			System.out.println("");
			System.out.println("");
			...
			if (result != null) {
				for (QueryStringScorePair q : result) {
					System.out.println(
							"==== [QueryRecommendatorFormTest::testRecommend] Rec. Query, Score:=" + q.getScore());
