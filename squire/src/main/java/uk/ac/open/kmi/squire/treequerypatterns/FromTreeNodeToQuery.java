 ...
 * depending on the variable and the template variable.
 * 
 * @author callocca
 */
public class FromTreeNodeToQuery<T> {
	...
	public void querySelectBuilding() {
		...
		List<TriplePath> tripleSet = (List<TriplePath>) this.node.getData();

		System.out.println("[FromTreeNodeToQuery::querySelectBuilding] This is the current triple patterns set "
				+ tripleSet.toString());

		// Building the triplePatternBlock as a conjtion of Triple patterns.
		List<String> resultVar = new ArrayList<>();
		...
		for (TriplePath tp : tripleSet) {
			// If the Subject is a variable Then it is potentially a Result variable of the
			// query
			...
			// If the Predicate is a variable Then it is potentially a Result variable of
			// the query
			...
			// If the Object is a variable Then it is potentially a Result variable of the
			// query
		...
		for (List<List<String>> partitions : compParts.partitions(resultVar, 0)) {
			// System.out.println("[FromTreeNodeToQuery::selectQueryBuilding] This is a new
			// partition");
