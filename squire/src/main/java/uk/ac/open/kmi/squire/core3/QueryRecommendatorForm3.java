 ...
 * @author carloallocca
 */
public class QueryRecommendatorForm3 {
	...
	public QueryRecommendatorForm3(String qString, IRDFDataset d1, IRDFDataset d2, float resultTypeSimilarityDegree,
	...
	public List<QueryStringScorePair> recommend() {
		...
		try {
			...
			throw new QueryParseException("[QueryRecommendatorForm::recommend] THE SOURCE QUERY is not parsable!!!", -1,
					-1);
		}
	...
	public List<QueryScorePair> recommendTest() {
		...
		if (qs.isSatisfiable(query, rdfd1)) {
			...
			//
			// System.out.println(" ");
			// System.out.println("[SQueryRecommendationWorker::queryRecommendation5] We are
			...
			// Collections.sort(qr.getQueryRecommendatedList(),
			// QueryScorePair.queryScoreComp);
