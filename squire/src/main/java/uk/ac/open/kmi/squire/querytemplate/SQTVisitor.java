...
public class SQTVisitor extends ElementVisitorBase implements IQueryVisitor {
    ...
    // In theory: The outer-most graph pattern in a query is called the query pattern. 
    // It is grammatically identified by GroupGraphPattern in "WhereClause ::=  'WHERE'? GroupGraphPattern"
    //ElementTriplesBlock is A SPARQL BasicGraphPattern. ex:
    // SELECT ?name ?mbox
    // WHERE  { // the outer-most graph pattern is a GroupGraphPattern - a set of graph pattern - and in jena is ElementPathBlock
    //          { ?x foaf:name ?name . } this is a BasicGraphPatterns - a set triple  pattern - and in jena is ElementTriplesBlock
    //          { ?x foaf:mbox ?mbox . } this is a BasicGraphPatterns - a set triple  pattern - and in jena is ElementTriplesBlock
    //        }
    // 
    ...
    public void visit(ElementPathBlock el) {
        ...
        while (it.hasNext()) {
            ...
            // Case 1: <ex:bob ex:hasSyster ex:alice >: this is in case I want to select the address of bobs that have a syster Alice  
            ...
            if (subj.isURI() && pred.isURI() && obj.isURI()) {
                ...
                //if subject is an individual and predicate=="rdf:type" and object is a Class then 
                // Questa mi seleziona l'insieme delle classi: select distinct ?class where {<subject> a ?class}
                // e per controllare se l'oggetto is a class basta controllare se e' nell'insieme appena calcolato.
                if(rdfd1.getIndividualSet().contains(subject)&& predicate.equals("http://www.w3.org/1999/02/22-rdf-syntax-ns#type")&&rdfd1.getClassSet().contains(object)){
                ...
                //if subject is an individual and predicate is an ObjectProperty and object is an individual then 
                if(rdfd1.getIndividualSet().contains(subject)&& rdfd1.getObjectPropertySet().contains(predicate)&&rdfd1.getIndividualSet().contains(object)){
                    Var individualVar = Var.alloc(individualVarTable.generateIFAbsentIndividualVar(subj.getURI()));
                    Var classVar = Var.alloc(classVarTable.generateIFAbsentClassVar(obj.getURI()));
                    it.set(new TriplePath(new Triple(individualVar, tp.getPredicate(), classVar)));
                }                
//                
//                System.out.println("subj.isURI() && pred.isURI() && obj.isURI()" + subj.getURI());
...
////                }
            ...
            // Case 2: <ex:fernando  ex:hasP2    "allocca" >
                ...
                //Var literalVar1 = Var.alloc( LiteralVarMapping.generateIFAbsentLiteralVar(obj.toString()));
