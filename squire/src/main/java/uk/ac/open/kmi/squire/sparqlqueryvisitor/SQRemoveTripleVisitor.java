...
//        System.out.println("[SQInstantiationVisitor::visit(ElementOptional el)] ");
    ...
    public void visit(ElementPathBlock el) {
        if (el == null) {
            throw new IllegalStateException("[SQRemoveTripleVisitor::visit(ElementPathBlock el)] The ElementPathBlock is null!!");
        }

        ListIterator<TriplePath> it = el.getPattern().iterator();
        while (it.hasNext()) {
            final TriplePath tp1 = it.next();
            System.out.println("[SQRemoveTripleVisitor::11111111111111111] TriplePath tp1 " + tp1.asTriple().toString());

            if (this.tp != null) {
                if (this.tp.matches(tp1.asTriple())) {
                    System.out.println("[SQRemoveTripleVisitor::3333333333333] this.tp.toString() " + this.tp.toString());

                    it.remove();
                } else {
                    System.out.println("[SQRemoveTripleVisitor::222222222222222222] this.tp.toString() " + this.tp.toString());
