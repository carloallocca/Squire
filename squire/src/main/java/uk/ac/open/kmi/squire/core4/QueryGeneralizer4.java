...
public class QueryGeneralizer4 {
	...
	public Query generalize() {
		...
		for (Node pred : predicates) {
			if (!(pred.isVariable()) && !(pred.isBlank())) {
				if (!this.rdfd2.getRDFVocabulary().contains(pred.getURI())) {
					log.debug("[:generalize] this is the predicate uri object that is going to be generalized "
							+ pred.getURI());
		...
		for (Node obj : objects) {
			if (!(obj.isVariable()) && !(obj.isBlank())) {
				log.debug("[QueryGeneralizer4::generalize] this is the uri object that is going to be generalized "
						+ obj.getURI());
	...
	private Var ifSubjectIsNotD2ThenGenerateVariableNew(Node subj) {
		if (subj == null || rdfd2 == null) {
			throw new IllegalStateException(
					"[QueryGeneralizer4::ifSubjectIsNotD2ThenGenerateVariable(Node subj)]The subj or rdfd2 is null!!");
		}
		final Var result;
		// SUBJECT
		if (subj.isURI()) {
	...
	private Var ifPredicateIsNotD2ThenGenerateVariableNew(Node pred) {
		...
		if (pred == null || rdfd2 == null) {
			throw new IllegalStateException(
					"[QueryGeneralizer4::ifPredicateIsNotD2ThenGenerateVariable(Node subj)]The subj or rdfd2 is null!!");
		}
		final Var result;
		if (pred.isURI()) {
	...
	private Var ifObjectIsNotD2ThenGenerateVariableNew(Node obj) {
		...
		if (obj == null || rdfd2 == null) {
			throw new IllegalStateException(
					"[QueryGeneralizer::ifSubjectIsNotD2ThenGenerateVariable(Node subj)]The subj or rdfd2 is null!!");
		}
		final Var result;
		// SUBJECT
		if (obj.isURI()) {
			...
			if ((rdfd1.getClassSet().contains(o)) && !(rdfd2.getClassSet().contains(o))) {
				log.debug("[QueryGeneralizer4::ifObjectIsNotD2ThenGenerateVariableNew] Here A "
						+ rdfd1.getClassSet().toString());
				...
				log.debug("[QueryGeneralizer4::ifObjectIsNotD2ThenGenerateVariableNew] Here E "
						+ rdfd1.getClassSet().toString());

				result = null;
				return result;
				// // this means that it is an individual or it could be present in both lists
				// (classes, obj and dtp).
	...
	private Set<Node> getSubjectsSet(Query originalQuery) {
		...
		if (this.originalQuery == null) {
			throw new IllegalStateException(
					"[QueryGeneralizer::getSubjectsSet(Query originalQuery)]The query is null!!");
		}
