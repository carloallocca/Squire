...
public class QueryStringScorePairResourceImplTest {
	...
	public void testArt() {
		...
		String qo1 = "SELECT distinct ?physicalThing ?hasType ?descr " + "WHERE { "
	...
	public void testEducationII() {
		...
		// ?opt1 ?this
		// }
		String qo2 = "SELECT distinct ?podcast ?download ?published ?this WHERE {"
		...
		String qo3 = "SELECT distinct ?patent ?label ?date ?title ?creator ?status ?authorList WHERE {"
		...
		String qo4 = "SELECT distinct ?building ?label ?buildName ?postCode WHERE {"
	...
	public void testMuseum() {
		String qo1 = "SELECT distinct ?place ?pLabel WHERE {" + "?place a <http://www.europeana.eu/schemas/edm/Place>."
	...
	public void testGovernmentOpenData() {
		String qo1 = "SELECT distinct ?s ?p ?o WHERE {"
				+ "?s a <http://data.ordnancesurvey.co.uk/ontology/admingeo/District>. " + "?s ?p ?o}";
		...
		// String target_endpoint = "http://data.admin.ch/sparql/"; //does not exist
		// anymore
