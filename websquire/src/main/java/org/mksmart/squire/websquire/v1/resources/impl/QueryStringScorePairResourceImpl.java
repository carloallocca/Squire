...
public class QueryStringScorePairResourceImpl implements QueryStringScorePairResource {
	...
	public JobStatement getRecommendedQueryList(@FormParam("qo") String qo,
			@FormParam("source_endpoint") String source_endpoint, @FormParam("target_endpoint") String target_endpoint,
		...
		// if((jobIndex1 instanceof SPARQLEndPoint && jobIndex1==d1) && (jobIndex2
		// instanceof
		...
		JobStatement jb;
		String token = jobMan.doQueryRecommendation(qo, d1, d2, resultTypeSimilarityDegree, queryRootDistanceDegree,
	...
	protected Object handlePromise(String token) throws WebApplicationException {
		...
		if (promise == null) {
			...
			throw new WebApplicationException(Response.serverError()
					.entity("Promise for token " + token + " could not be found. Has the indexing job been submitted?")
					.build());
		}
		Object jobIndex;
		try {
